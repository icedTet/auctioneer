...
export class SPSBAuction extends FPSBAuction implements Auction {
    ...
    constructor(buyers: Buyer[], secondPlaceBuyers: BuyerSecondPlace[], itemValue: number) {
    ...
    results(winRequirement: number, full: boolean) {
        ...
        const sortedSecondPlaceBuyers = Array.from(this.secondPlaceBuyers.values()).sort(
            (a, b) => b.satiety - a.satiety
        )
        ...
        sortedSecondPlaceBuyers.forEach((secondPlaceBuyer) => {
            satietyMap.set(secondPlaceBuyer.satiety, (satietyMap.get(secondPlaceBuyer.satiety) || 0) + 1);
